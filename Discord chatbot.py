...
# Respond only in the allowed channel or in DMs
@bot.event
async def on_message(message):
    ...
    # If it's a DM, the bot should always respond
    if isinstance(message.channel, discord.DMChannel):
        user_input = message.content.strip()
        print(f"User (DM): {user_input}")

        # Prepare payload for AI response
        payload = {
            "model": MODEL_NAME,
            "messages": [
                {"role": "system", "content": "You are a friendly and casual AI who chats like a normal human."},
        ...
        try:
            ...
            print(f"Bot (DM): {ai_response}")

            # Send the AI's response back to the DM
            await message.author.send(ai_response)

        except Exception as e:
            print(f"Error: {e}")
            await message.author.send("Oops, something went wrong while trying to respond!")

    # If it's a regular channel, check if the allowed channel is set
    elif allowed_channel_id and message.channel.id != allowed_channel_id:
        return  # Don't respond if the message is in an unauthorized channel

    # For messages in the allowed channel
    else:
        user_input = message.content.strip()
